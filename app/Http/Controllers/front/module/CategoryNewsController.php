...
class CategoryNewsController extends Controller
{
    public function index($id,$setting)
    {
        ...
        {
            $mnu = \App\Models\Menu::where('id', $setting->menu)->first();
            $datas['menu_href'] = '';
            if ($mnu)
            {
                $datas['menu_href'] = $mnu->seo_url;
            }
            $art = ArticleToMenu::leftJoin('article','article_to_menu.article_id', '=', 'article.id')
                ...
                ->where('article_to_menu.menu_id', $setting->menu)
